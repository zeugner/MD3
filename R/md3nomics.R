...
#' @param pat pattern to match against the dataflow id or description. This uses in-built REgex expresions, but remains case-insensitive. If a pattern is not provided, all dataflows (or codes with labels) are returned .
...
helpNomics = function(query, pattern="", dim=NULL, verbose=TRUE) {
  ...
  if (length(query)==2L & !length(dim)) {
    if (nchar(pat)>0L) {
      ...
      str=str[grepl(pat,rownames(str),ignore.case = TRUE) | grepl(pat,str[,1],ignore.case = TRUE),,drop=FALSE]
      if (!NROW(str)) {stop('No code matching pattern ', pat, ' can be found in dimension ',dim,' of ', query[[1]],'/',query[[2]])}
